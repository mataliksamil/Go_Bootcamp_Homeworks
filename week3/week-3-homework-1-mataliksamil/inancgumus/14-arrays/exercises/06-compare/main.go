...
func main() {
	...
	//wend := [4]string{"Saturday", "Sunday"}// not comparable since arrays will be having different sizes
	wend := [2]string{"Saturday", "Sunday"}
	fmt.Println(week != wend)

	evens := [...]int{2, 4, 6, 8, 10}
	//evens2 := [...]int32{2, 4, 6, 8, 10} // not comparable since not have the same (int) type
